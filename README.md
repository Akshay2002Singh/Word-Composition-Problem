...
## *Code Structure*
- 'trie_node' and 'trie' classes are defined to create and manage a trie data structure.
- The 'read_file' function reads words from the selected input file into a vector.
- The 'comparator' function is used to sort the input list based on string length.
- The check_compound function recursively checks whether a word is compound or not.
- In the main function, we perform following operations:

    - Reads the input file, and fill vector with words.
    - Sort vector on basis of word length.
    - Builds a trie.
    - Identifies the longest and second longest compounded words.
    - Print output.

## *Example Output*
The program will display the following information in the console:
```
Longest Compound Word: ratcatdogcat
Second Largest Compound Word: catsdogcats
Time Taken to Process Input File: 1 milli seconds
```

## *Conclusion*
This C++ program efficiently identifies the longest and second longest compounded words within the input files. It uses a trie data structure and a recursive approach for correctness and performance. This tool is valuable for solving the given problem and can be adapted for various applications that involve word composition.